          ...
          mongo.db.collection('replies')
            ...
            .toArray((err, result) => {
              if(err){console.log(err)}else {
                ...
                thread.lcTopic = thread.topic
                thread.topic = thread.topic.capitalizeFirstLetter()
                // FIX (XSS): removed the broken case-sensitive single-replace sanitizer
                // (e.g. `<Script` bypassed it).  The view now uses {{ }} double-brace
                // mustache syntax which HTML-entity-escapes output automatically, making
                // server-side stripping unnecessary.  No mutation of thread.body needed.
