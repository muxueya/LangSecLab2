...
// FIX (Prototype Pollution): block keys that traverse the prototype chain.
// The original merge iterated all keys including __proto__, constructor, and prototype,
// which allowed an attacker to write arbitrary properties onto Object.prototype.
// That in turn triggered the eval() gadget in passport.js (RCE).
const UNSAFE_KEYS = new Set(['__proto__', 'constructor', 'prototype']);

function merge(target, source) {
  Object.keys(source).forEach(key => {
    if (UNSAFE_KEYS.has(key)) return;   // skip dangerous keys
...
function validatePassword(req, res, next) {
  ...
  // Previously `name.match(pass)` used the new password as a regex pattern,
  // which could be abused to freeze the event loop with a crafted regex payload.
  if (name.toLowerCase().includes(pass.toLowerCase())) {
...
router
  ...
  // FIX (Unrestricted File Upload / XSS CSP bypass): added loginRequired so only
  // authenticated users can upload; previously any anonymous request could upload
  // an arbitrary file, which an attacker could then load as a same-origin script.
  ...
  // FIX (Prototype Pollution / RCE): added adminRequired.
  // Previously any unauthenticated request could POST to /upload/users and upload
  // a JSON payload containing "__proto__" to pollute Object.prototype, then trigger
  // the eval() gadget in passport.js to achieve full RCE.
